...
define([
    ...
    async function post(requestBody) {
        try {
            ...
            log.debug('post 요청__ll_create parsedBody ==== ', parsedBody);
            ...
            log.debug("!!!submitLogRecord ---- ", submitLogRecord)
        ...
        // subsidiary_id, transaction_id
            ...
            if (paramArray) {
                for (let obj of paramArray) {
                    ...
                    transactionFilters.push([prefix + 'flag', search.Operator.ISNOT, '1']); // flag 값이 '1'이나 '2'가 아닌 것 (bulkSbumit 또는 getBulkSumbitResult 호출하지 않은 것)
                    transactionFilters.push('or')
                    transactionFilters.push([prefix + 'flag', search.Operator.ISNOT, '2']); // flag 값이 '1'이나 '2'가 아닌 것 (bulkSbumit 또는 getBulkSumbitResult 호출하지 않은 것)
                    transactionFilters.push('or')
                }
